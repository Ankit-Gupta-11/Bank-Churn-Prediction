...
def optimize(params, param_names, x, y, model_name):
...
def get_best_params(clf): 
    ...
    if clf == "xgboost":
        ...
        df = pd.read_csv(os.path.join(config.OUTPUTS, f"{config.FILE_NAME}_processed.csv"))
        X = df.drop(['kfold', config.TARGET_VARIABLE], axis = 1)
        y = df[config.TARGET_VARIABLE]
        optimize_function = partial(optimize, param_names = param_names, x = X, y = y, model_name = clf)
