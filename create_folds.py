...
if __name__ == '__main__':
    ...
    df = pd.read_csv(os.path.join(config.OUTPUTS, f"{config.FILE_NAME}_processed.csv")).sample(frac = 1).reset_index(drop = True)
    df['kfold'] = -1
    ...
    df.to_csv(os.path.join(config.OUTPUTS, f"{config.FILE_NAME}_kfold.csv"), index = False)